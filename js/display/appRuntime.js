/**
 * HALDO AI OS 24.6.0 – APP RUNTIME
 */
const AppRuntime = {
    runningApps: {},
    appStates: {},
    ...
    start(appId, params = {}) {
        ...
        this.runningApps[appId] = { id: appId, startedAt: Date.now(), status: 'running', params };
        if (!this.appStates[appId]) this.appStates[appId] = { data: {}, settings: {} };
        if (typeof app.start === 'function') app.start(params);
    ...
    stop(appId) {
        const app = AppRegistry.get(appId);
        if (!app) return false;
        if (!this.runningApps[appId]) { console.warn(`⚠️ App ${appId} läuft nicht`); return false; }
    ...
    getRunningApps() { return Object.values(this.runningApps); },
    isRunning(appId) { return !!this.runningApps[appId]; },
    getState(appId) { return this.appStates[appId] || null; },
    ...
    restart(appId) { this.stop(appId); setTimeout(() => this.start(appId), 200); return this; },
    stopAll() { Object.keys(this.runningApps).forEach(id => this.stop(id)); return this; }
};
